...
const Concluidos = () => {
  ...
  const pesquisaNome = (clienteId) => {
    axios.get(`https://projeto.viniciuscavalc6.repl.co/api/alugueis?sort=data_devolucao:desc&filters[concluido]=true&filters[cliente][id][$eq]=${clienteId}&populate=*`, config)
      .then((response) => {
        if (response.status === 200) {
          const dados = response.data.data;
          let dadosProcessados = dados.map((Aluguel) => {
